...
router.get('/tags/popular', async (req, res) => {
    try {
        // Unnest tags, group by tag, count occurrences, and take the top 10
        const sql = `
...
router.get('/:id', optionalProtect, async (req, res) => {
    try {
        ...
        // If the post is a draft, only return it to the author.
...
router.post('/:id/like', protect, async (req, res) => {
    try {
        ...
        // Handle potential primary key violation if two requests arrive at once
...
router.delete('/:postId/comments/:commentId', protect, async (req, res) => {
    try {
        ...
        const intId = parseInt(commentId, 10);
        if (Number.isNaN(intId)) return res.status(404).json({ message: 'Comment not found' });

        const userId = req.user.id;
        const del = await db.query('DELETE FROM comments WHERE id = $1 AND user_id = $2 RETURNING id', [intId, userId]);
        if (del.rows.length === 0) return res.status(404).json({ message: 'Comment not found or not authorized' });
